import '../../../../lib/engine/adapters'; // register providers so getExecutor resolves
import { getDb } from '../../../../lib/db';
import { getLiveCatalog } from '../../../../lib/agents/live-catalog';
import { getExecutor } from '../../../../lib/engine/executor';

export const runtime = 'nodejs';
export const maxDuration = 180;

/**
 * GET /api/models/health — does every configured model ACTUALLY render?
 *
 * Why this exists: FLUX failed for weeks and read as a flaky connection. The real cause was an input
 * SHAPE mismatch — the adapter sent `input_image`/`input_image_2` where the model defines a single
 * `input_images` array — so text-only worked and every reference render failed. Nothing in the
 * system could tell the difference between "provider is down" and "we are calling it wrong", and no
 * test could catch it: unit tests mock the provider, and that's exactly the layer that was lying.
 *
 * So this spends real (tiny) money on purpose: one minimal render per model, optionally a second
 * with a reference, reporting each provider's own error message. `?refs=1` exercises the reference
 * path — the one that actually broke. `?model=<id>` checks one model.
 *
 * NOT called automatically anywhere: it costs money, so it stays a deliberate operator action.
 */
const TINY_PROMPT = 'a plain red circle centered on a white background';
/**
 * A SELF-CONTAINED 64×64 gradient PNG for exercising the reference path.
 *
 * It was a hosted URL, and hosted test images rot: provider CDN links expire, so a green check could
 * turn red overnight and read as an adapter regression that never happened. A health check whose own
 * fixture can break is a health check you stop trusting. Inline data means the only thing under test
 * is our request shape.
 */
const REF_IMAGE = 'data:image/png;base64,';

interface Check {
  modelId: string;
  label: string;
  provider: string;
  mode: 'text' | 'reference';
  ok: boolean;
  ms: number;
  /** The provider's own reason when it failed — the detail that turns "flaky" into a diagnosis. */
  error?: string;
  url?: string;
}

async function checkOne(
  model: { id: string; label: string; provider: string; aspectRatios: string[] },
  mode: 'text' | 'reference',
): Promise<Check> {
  const base: Omit<Check, 'ok' | 'ms'> = { modelId: model.id, label: model.label, provider: model.provider, mode };
  const exec = getExecutor(model.provider as never);
  const t0 = Date.now();
  if (!exec) return { ...base, ok: false, ms: 0, error: 'no adapter registered' };
  if (!exec.isConfigured()) return { ...base, ok: false, ms: 0, error: 'no API key configured' };

  try {
    for await (const ev of exec.generate({
      modelId: model.id,
      prompt: TINY_PROMPT,
      n: 1,
      aspectRatio: model.aspectRatios.includes('1:1') ? '1:1' : model.aspectRatios[0],
      ...(mode === 'reference' ? { references: [REF_IMAGE] } : {}),
    } as never)) {
      if (ev.type === 'tile') return { ...base, ok: true, ms: Date.now() - t0, url: ev.image.url };
      if (ev.type === 'error') {
        // Distinguish "your key is bad" from "our request shape is wrong" — the whole point of this
        // endpoint is to end that ambiguity, so the reason is spelled out rather than left as a code.
        const hint =
          ev.reason === 'no_key'
            ? 'auth rejected — check the provider API key / account'
            : ev.reason === 'bad_request'
              ? 'provider rejected the REQUEST SHAPE — an adapter parameter is wrong'
              : ev.reason;
        const detail = (ev as { detail?: string }).detail;
        return { ...base, ok: false, ms: Date.now() - t0, error: detail ? `${hint} · ${detail}` : hint };
      }
    }
    return { ...base, ok: false, ms: Date.now() - t0, error: 'stream ended with no image' };
  } catch (err) {
    return { ...base, ok: false, ms: Date.now() - t0, error: err instanceof Error ? err.message : 'threw' };
  }
}

export async function GET(req: Request) {
  const url = new URL(req.url);
  const only = url.searchParams.get('model');
  const withRefs = url.searchParams.get('refs') === '1';

  const db = await getDb();
  const catalog = await getLiveCatalog(db);
  const targets = catalog
    .filter((m) => !m.preview && !m.needsResearch)
    .filter((m) => !only || m.id === only);

  if (targets.length === 0) return Response.json({ error: `no callable model matched${only ? ` "${only}"` : ''}` }, { status: 404 });

  // Sequential: this spends money and hits rate limits — a health check that trips the provider's
  // limiter would report failures it caused itself.
  const checks: Check[] = [];
  for (const m of targets) {
    checks.push(await checkOne(m, 'text'));
    // Only exercise references on models that actually accept them.
    if (withRefs && m.maxReferenceImages > 0) checks.push(await checkOne(m, 'reference'));
  }

  const failed = checks.filter((c) => !c.ok);
  return Response.json({
    healthy: failed.length === 0,
    checked: checks.length,
    failed: failed.length,
    checks,
  });
}
